...
export default function CareerAgent() {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const botResponse = json.data.response;
      const meta = json.data.execution_metadata;
      
      setMessages((prev) => [...prev, { role: "assistant", content: botResponse }]);
      
      if (meta && meta.agent_traces && meta.agent_traces.length > 0) {
        const traceStr = meta.agent_traces.join("\n");
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFileLoading(true);
    setError(null);
    setMessages((prev) => [...prev, { role: "user", content: "[Uploaded CV: " + file.name + "]" }]);
    ...
    try {
      ...
      const json = await res.json();
      setMessages((prev) => [...prev, { role: "assistant", content: `✅ CV uploaded! I've extracted ${json.char_count} characters. You can now ask me questions about it.` }]);
        ...
        <header className="h-16 flex items-center justify-between px-6 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-slate-800/50 z-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              disabled={fileLoading || isLoading || !sessionId}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 scroll-smooth bg-gradient-to-b from-[#0a0a0a] to-[#111]">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-slate-500 gap-4 opacity-50">
              ...
              {!sessionId && <p className="text-xs animate-pulse text-indigo-400">Connecting to secure Vercel API...</p>}
            </div>
          )}
            ...
            {isLoading && (
              ...
              >
                ...
                <div className="px-5 py-4 rounded-2xl bg-[#1a1a1a] border border-slate-800 rounded-tl-sm flex items-center gap-2">
                  ...
                  <span className="text-sm text-slate-400">Analyzing skills via Vercel Backend...</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="p-4 bg-[#0a0a0a] border-t border-slate-800/50">
          ...
          >
            <input
              ...
              disabled={isLoading || fileLoading || !sessionId}
            ...
            <button
              type="submit"
              disabled={!input.trim() || isLoading || fileLoading || !sessionId}
          ...
          <p className="text-center text-xs text-slate-600 mt-3">
            Powered by Vercel Serverless (Next.js + Python FastAPI).
          </p>
        </div>
      </div>
